...
export type WithCertType<T extends CertificateModel> = T & {
  type: 'retire' | 'update' | 'mir' | 'stake' | 'delegation';
};
...
export interface PoolUpdateCertModel extends CertificateModel {
  pool_id: string;
}
export const isPoolUpdateCertModel = (
  value: WithCertType<CertificateModel>
): value is WithCertType<PoolUpdateCertModel> => value.type === 'update';
