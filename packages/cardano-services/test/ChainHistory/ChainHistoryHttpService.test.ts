...
import { Cardano, ChainHistoryProvider, ProviderError, ProviderFailure, util } from '@cardano-sdk/core';
  ...
  describe('healthy state', () => {
    ...
    describe('/health', () => {
      ...
      it('forwards the ChainHistoryProvider health response', async () => {
        const res = await got(`${apiUrlBase}${url}`, { headers: { 'Content-Type': APPLICATION_JSON } });
    ...
    describe('/blocks/hash', () => {
      ...
      it('returns a 200 coded response with a well formed HTTP request', async () => {
        expect((await got.post(`${apiUrlBase}${url}`, { json: { args: [[]] } })).statusCode).toEqual(200);
      });

      it('returns a 415 coded response if the wrong content type header is used', async () => {
        try {
          await got.post(`${apiUrlBase}${url}`, { headers: { 'Content-Type': APPLICATION_CBOR } });
    ...
    describe('/txs/address', () => {
      const url = '/txs/address';
      it('returns a 200 coded response with a well formed HTTP request', async () => {
        expect((await got.post(`${apiUrlBase}${url}`, { json: { args: [] } })).statusCode).toEqual(200);
      });
      ...
      describe('server and snapshot testing', () => {
        it('finds transactions with address within inputs', async () => {
          ...
          const response = await doChainHistoryRequest<[Cardano.Address[]], Cardano.TxAlonzo[]>(url, [addresses]);
          expect(response).toHaveLength(1);
          expect(response).toMatchSnapshot();
        });
        ...
        it('does not include transactions before indicated block', async () => {
          ...
          const response = await doChainHistoryRequest<[Cardano.Address[], number], Cardano.TxAlonzo[]>(url, [
            addresses,
            1_654_555
          ]);
          expect(response.length).toEqual(2);
